...
本目录的 `genshen-cp4.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp4.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp4 deepking          # 导出 ~/.genshen-cp4/deepking/*.skin.json
  ```
...
```bash
genshen-cp4 deepking --check    # 应输出「通过」
genshen-cp4 deepking --what     # 看转换器实际会取到什么值
```
...
- `mascot-cp4-light.jpg` —— 取上半身特写（纳西妲与安柏的脸），配浅色界面
- `mascot-cp4-dark.jpg` —— 取全身与森林构图（绿林与黄花），配深色界面

两张都由 `01-nahida.jpg` 生成。重做方式：调整 `crop` 的中心比例与取景边长，
