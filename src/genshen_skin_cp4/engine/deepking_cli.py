...
原神 CP 壁纸套件 5 —— DeepKing 皮肤命令行
  ...
  genshen-cp4 deepking --what       # 只显示 DeepKing 会从仓库里提取到什么
  genshen-cp4 deepking --out DIR    # 指定输出目录
  genshen-cp4 deepking --css        # 额外导出「按 CSS 变量推导」的那一版(与 DeepKing 在线转换一致)
"""
import argparse
import json
import os
...
def main(argv=None):
    ...
    ap.add_argument("--out", default=None, help="输出目录(默认 ~/.genshen-cp4/deepking)")
