...
const REPO_URL = 'https://github.com/WPH666-py/Genshen-skin-CP4.git';
const EXT_ID = 'wp666.genshen-skin-cp4';
...
function detectPython() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp4').get('pythonPath', ''));
...
function findRepo() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp4').get('repoPath', ''));
  const cands = [
    cfg,
    process.env.GENSHIN_CP1_REPO,
    path.join(os.homedir(), 'Genshen-skin-CP4'),
    process.env.USERPROFILE ? path.join(process.env.USERPROFILE, 'Genshen-skin-CP4') : null,
  ...
  for (const c of cands) {
    try {
      if (fs.existsSync(path.join(c, 'src', 'genshen_skin_cp4', 'cli.py'))) return c;
...
function buildInvocation(sub) {
  ...
  if (repo) {
    env.PYTHONPATH = (env.PYTHONPATH ? env.PYTHONPATH + path.delimiter : '') +
      path.join(repo, 'src');
    return { py: py, args: ['-m', 'genshen_skin_cp4.cli'].concat(sub), cwd: repo, env: env };
  }
  // pip 安装形态: 直接用模块
  return { py: py, args: ['-m', 'genshen_skin_cp4.cli'].concat(sub), cwd: os.homedir(), env: env };
}

function envError(msg) {
  vscode.window.showWarningMessage(
    msg + ' 请先安装: pip install ' + 'genshen-skin-cp4' +
    '  (或克隆 ' + REPO_URL + ' 到 ' + path.join(os.homedir(), 'Genshen-skin-CP4') + ')'
  );
}
...
function spawnGui(moduleName, label) {
  ...
  const child = spawn(exe, ['-m', 'genshen_skin_cp4.' + moduleName], {
    cwd: inv.cwd,
    env: inv.env,
    detached: true,
    stdio: 'ignore',
...
function activate(ctx) {
  ...
  reg('genshencp4.setCard', () => runCli(['card'], '已切换: 卡片式'));
  reg('genshencp4.setCover', () => runCli(['cover'], '已切换: 满屏'));
  reg('genshencp4.setShowall', () => runCli(['showall'], '已切换: 完整不裁'));
  reg('genshencp4.setRandom', () => runCli(['random'], '已随机换一张!'));
  reg('genshencp4.generateAll', () => runCli(['all'], '已生成全部样式!'));
  reg('genshencp4.openSwitcher', () => spawnGui('switcher', '壁纸切换器'));
  reg('genshencp4.startPet', () => spawnGui('pet', '桌面桌宠'));
  reg('genshencp4.showInfo', () => {
    const inv = buildInvocation(['info']);
    if (inv.error) return envError(inv.error);
    const term = vscode.window.createTerminal({ name: '原神CP4 自检', cwd: inv.cwd, env: inv.env });
    term.show();
    term.sendText('"' + inv.py + '" ' + inv.args.map((a) => '"' + a + '"').join(' '));
  });
  reg('genshencp4.openGallery', () =>
    vscode.commands.executeCommand('workbench.view.extension.genshencp4')
  );

  ctx.subscriptions.push(
    vscode.window.registerWebviewViewProvider('genshencp4.gallery', new GalleryProvider(), {
